...
object DomainToEntityMapper {
  ...
  fun Race.toEntity(season: Season) = RaceEntity(
    id = id().value,
    name = name().value,
  ...
  fun RaceResult.toEntity(season: Season, race: Race) = RaceResultEntity(
    id = id,
    race = RaceEntity(
      id = race.id().value,
  ...
  fun Elo.toEntity(driver: Driver) = DriverEloHistoryEntity(
    elo = rating,
